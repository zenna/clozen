...
;; n-dimensional intervals, or orthotopes, or simply boxes.
(defrecord Box
  [n-dims intervals])
...
(defn ->Box
  "Box Constructor"
  [intervals]
  {:pre [(every? true? (map (fn [[low up]] (<= low up)) intervals))]}
  (Box. (count intervals) intervals))
...
(defn middle-split
  ...
  [{box :intervals}]
  (mapv #(double (+ (lower-bound %) (/ (- (upper-bound %) (lower-bound %)) 2))) box))
...
(defn split
  ...
  (mapv
    ->Box
    (for [dim-to-change (apply combo/cartesian-product (:intervals box))]
      (mapv
        (fn [dim-to-replace min-max split-point]
          (vec (sort [(first (filter #(not= dim-to-replace %) min-max)) split-point])))
        dim-to-change (:intervals box) split-points))))
...
(defn abstraction-vertices
  ...
  [box]
  (vec (apply combo/cartesian-product (:intervals box))))
...
(defn point-in-box?
  ...
  [box point]
  {:pre [(count= (num-dims box) point)]}
  (intersect? box (->Box (mapv #(vector % %) point))))
    ...
    (loop [n-samples n-samples]
      ...
      (cond
        ...
        (let [sample (abstraction-sample exp-box)]
          (if (p :pia (some #(point-in-box? % sample) boxes))
              (recur (dec n-samples))
              false))))))
...
;;;;;;;;;;;; This file autogenerated from src/cljx/clozen/geometry/box.cljx
